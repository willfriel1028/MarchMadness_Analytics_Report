...
def team_matchup(team1, team2, data_test):
    ...
    wab1, wab2, esos1, esos2 = sos_matchup(team1, team2, data_test)    
        # Calls sos_matchup and saves the returns into variables
    tempo1, tempo2, use_imp = tempo_matchup(team1, team2, data_test)   
        # Calls tempo_matchup and saves the returns into variables
    oe1, oe2, de1, de2, efgo1, efgo2, efgd1, efgd2, odnet1, odnet2 = off_def_matchup(team1, team2, data_test)   
        # Calls off_def_matchup and saves the returns into variables
    p2imp1, p2imp2, p2eff1, p2eff2, p2netimp1, p2netimp2, p2neteff1, p2neteff2 = twopt_matchup(team1, team2, data_test)
        # Calls twopt_matchup and saves the returns into variables
    p3imp1, p3imp2, p3eff1, p3eff2, p3netimp1, p3netimp2, p3neteff1, p3neteff2 = threept_matchup(team1, team2, data_test)
        # Calls threept_matchup and saves the returns into variables
    ftimp1, ftimp2, fteff1, fteff2, ftnetimp1, ftnetimp2, ftneteff1, ftneteff2 = ft_matchup(team1, team2, data_test)
        # Calls ft_matchup and saves the returns into variables
    to1, to2, tod1, tod2, to1net, to2net = to_matchup(team1, team2, data_test)
        # Calls to_matchup and saves the returns into variables
    hgt1, hgt2, orb1, orb2, drb1, drb2, rb1net, rb2net = reb_matchup(team1, team2, data_test)
        # Calls reb_matchup and saves the returns into variables
    
    if use_imp:    # If use_imp=True, we calculate win percentages using impact scores
        get_win_percentages(team1, team2, wab1, wab2, odnet1, odnet2, p2netimp1, p2netimp2, p3netimp1, p3netimp2, ftnetimp1, ftnetimp2, to1net, to2net, rb1net, rb2net, seedA, seedB, data_test) # Calls get_win_percetages
    else:          # If use_imp=False, we calculate win percentages using efficiency scores
        get_win_percentages(team1, team2, wab1, wab2, odnet1, odnet2, p2neteff1, p2neteff2, p3neteff1, p3neteff2, ftneteff1, ftneteff2, to1net, to2net, rb1net, rb2net, seedA, seedB, data_test) # Calls get_win_percentages
...
def sos_matchup(team1, team2, data_test):
    ...
    return teamA_wab, teamB_wab, teamA_esos, teamB_esos # Returns each team's Elite SOS and WAB
    
def check_tempo(aT, bT):
    '''
    This function checks whether the 2 team's tempo is similar or not
    The function reads in:
        aT (team1's tempo)
        bT (team2's tempo)
    Used to determine whether or not to use impact scores or efficiency scores
    '''
    # Checks to see if the two teams' tempos are within 5 of each other
    if abs(aT - bT) <= 5:
        return True # If they are within 5
    else:
        return False # If they are not within 5
...
def tempo_matchup(team1, team2, data_test):
    ...
    The function reads in:
        ...
        data_test (the dataset for the year the user input)
    The function calls:
        check_tempo - to determine whether impact or efficiency scores will be used for calculations/analyzed
    Returns each team's tempo and boolean use_impact
    '''
    ...
    use_impact = check_tempo(teamA_tempo, teamB_tempo) # Calls check_tempo
    
    # DISPLAYS EACH TEAMS' TEMPO AND WHETHER TO LOOK AT IMPACT OR EFFICIENCY SCORES
    print("\n\n\nTEMPO COMPARISON")
    print()
    print("ADJUSTED TEMPO")
    print(f"{team1 + ':':<12} {teamA_tempo:.2f}    {team2 + ':':<12} {teamB_tempo:.2f}") # Displays each teams' tempo
    if use_impact: # Tempos within 5
        print("Suggestion: Look at the teams' respective Impact Scores") # Use/look at impact scores (Game tempo is predictable)
    else: # Tempos not within 5
        print("Suggestion: Look at the teams' respective Efficiency Scores") # Use/look at efficiency scores (Game tempo is not predictable)
        
    return teamA_tempo, teamB_tempo, use_impact # Returns tempos and use_impact
...
def off_def_matchup(team1, team2, data_test):
    ...
    return teamA_OE, teamB_OE, teamA_DE, teamB_DE, teamA_efgO, teamB_efgO, teamA_efgD, teamB_efgD, AB_net, BA_net # Returns everything
    
def twopt_matchup(team1, team2, data_test):
    '''
    ...
    Retrieves each team's 2pt offensive and defensive metrics.
    Calculates how each team's 2pt offense matches up against the other's defense.
    Normalizes the matchup scores to a [-1,1] scale for both impact and efficiency.
    ...
    # Find the min and max values across the dataset for all offensive and defensive 2pt metrics
    ...
    min_impact_matchup = np.log(min_off_impact * min_def_impact) # Worst matchup (worst offense vs best defense)
    max_impact_matchup = np.log(max_off_impact * max_def_impact) # Best matchup (best offense vs worst defense)
    min_eff_matchup = np.log(min_off_eff * min_def_eff) # Worst efficiency matchup
    max_eff_matchup = np.log(max_off_eff * max_def_eff) # Best efficiency matchup
    
    # Retrieve both teams' individual 2pt stats for offense, defense, net scores
    ...
    AB_impact = np.log(teamA_off_impact * teamB_def_impact) # team1’s offense vs team2’s defense (impact)
    AB_net_impact = 2 * ((AB_impact - min_impact_matchup) / (max_impact_matchup - min_impact_matchup)) - 1 # Normalize to [-1,1]
    BA_impact = np.log(teamB_off_impact * teamA_def_impact) # team2’s offense vs team1’s defense (impact)
    BA_net_impact = 2 * ((BA_impact - min_impact_matchup) / (max_impact_matchup - min_impact_matchup)) - 1 # Normalize to [-1,1]
    AB_eff = np.log(teamA_off_eff * teamB_def_eff) # team1’s offense vs team2’s defense (efficiency)
    AB_net_eff = 2 * ((AB_eff - min_eff_matchup) / (max_eff_matchup - min_eff_matchup)) - 1 # Normalize to [-1,1]
    BA_eff = np.log(teamB_off_eff * teamA_def_eff) # team2’s offense vs team1’s defense (efficiency)
    BA_net_eff = 2 * ((BA_eff - min_eff_matchup) / (max_eff_matchup - min_eff_matchup)) - 1 # Normalize to [-1,1]
    
    # DISPLAY ALL STATS AND MATCHUPS
    print("\n\n\nTWO POINT COMPARISON")
    print()
    print("NET 2PT IMPACT SCORE (Shows estimated 2pt margin for team given adjusted tempo)") # Net margin using impact score
    print(f"{team1 + ':':<12} {teamA_net_impact:.2f}    {team2 + ':':<12} {teamB_net_impact:.2f}") # Show both teams’ net impact
    print()
    print("NET 2PT EFFICIENCY SCORE (Shows estimated 2pt margin for team given a fixed tempo)") # Net margin using fixed tempo
    print(f"{team1 + ':':<12} {teamA_net_eff:.2f}    {team2 + ':':<12} {teamB_net_eff:.2f}") # Show both teams’ net efficiency
    print()
    print("HEAD TO HEAD MATCHUP")
    print("OFFENSE VS DEFENSE IMPACT SCORE (Closer to -1: Disadvantage, Closer to 0: Even, Closer to 1: Advantage)") # Explain scale
    print(f"{team1 + ':':<12} {AB_net_impact:.2f}    {team2 + ':':<12} {BA_net_impact:.2f}") # Show impact-based matchup scores
    print()
    print("OFFENSE VS DEFENSE EFFICIENCY SCORE (Closer to -1: Disadvantage, Closer to 0: Even, Closer to 1: Advantage)") # Explain scale
    print(f"{team1 + ':':<12} {AB_net_eff:.2f}    {team2 + ':':<12} {BA_net_eff:.2f}") # Show efficiency-based matchup scores
    
    # RETURN EVERYTHING
    return teamA_net_impact, teamB_net_impact, teamA_net_eff, teamB_net_eff, AB_net_impact, BA_net_impact, AB_net_eff, BA_net_eff
    
def threept_matchup(team1, team2, data_test):
    '''
    This function analyzes and displays how both teams perform in 3-point shooting.
    The function reads in:
        team1 (the first team the user input)
        team2 (the second team the user input)
        data_test (the dataset for the year the user input)
    Calculates the best and worst possible 3pt matchups using log-scaled values.
    Retrieves each team's 3pt offensive and defensive metrics.
    ...
    # Find the min and max values across the dataset for all offensive and defensive 3pt metrics
    ...
    # Calculate best and worst possible matchups using log(offense × defense)
    min_impact_matchup = np.log(min_off_impact * min_def_impact) # Worst impact matchup
    max_impact_matchup = np.log(max_off_impact * max_def_impact) # Best impact matchup
    min_eff_matchup = np.log(min_off_eff * min_def_eff) # Worst efficiency matchup
    max_eff_matchup = np.log(max_off_eff * max_def_eff) # Best efficiency matchup
    
    # Retrieve both teams' individual 3pt stats for offense, defense, net scores
    ...
    print()
    print("NET 3PT IMPACT SCORE (Shows estimated 3pt margin for team given adjusted tempo)") # Net margin using impact score
    print(f"{team1 + ':':<12} {teamA_net_impact:.2f}    {team2 + ':':<12} {teamB_net_impact:.2f}") # Show both teams’ net impact
    print()
    print("NET 3PT EFFICIENCY SCORE (Shows estimated 3pt margin for team given a fixed tempo)") # Net margin using fixed tempo
    print(f"{team1 + ':':<12} {teamA_net_eff:.2f}    {team2 + ':':<12} {teamB_net_eff:.2f}") # Show both teams’ net efficiency
    print()
    print("HEAD TO HEAD MATCHUP")
    print("OFFENSE VS DEFENSE IMPACT SCORE (Closer to -1: Disadvantage, Closer to 0: Even, Closer to 1: Advantage)") # Explain scale
...
def ft_matchup(team1, team2, data_test):
    '''
    This function analyzes and displays how both teams perform in the Free Throw game.
    The function reads in:
        team1 (the first team the user input)
        team2 (the second team the user input)
        data_test (the dataset for the year the user input)
    Calculates the best and worst possible FT matchups using log-scaled values.
    Retrieves each team's FT offensive and defensive metrics.
    ...
    # Find min and max values across the dataset for all FT-related offensive and defensive metrics
    ...
    # Calculate best and worst possible matchups using log(offense × defense)
    min_impact_matchup = np.log(min_off_impact * min_def_impact) # Worst possible impact matchup
    max_impact_matchup = np.log(max_off_impact * max_def_impact) # Best possible impact matchup
    min_eff_matchup = np.log(min_off_eff * min_def_eff) # Worst possible efficiency matchup
    max_eff_matchup = np.log(max_off_eff * max_def_eff) # Best possible efficiency matchup

    # Impact: FT% * FTR * AdjT * (1 - TOV%)
    # Efficiency: FT% * FTR * 66 * (1 - TOV%)
    # Retrieve both teams' individual FT stats
    ...
    # Calculate log-scaled matchup strength for FT impact and efficiency for each team
    AB_impact = np.log(teamA_off_impact * teamB_def_impact) # team1’s FT offense vs team2’s FT defense (impact)
    AB_net_impact = 2 * ((AB_impact - min_impact_matchup) / (max_impact_matchup - min_impact_matchup)) - 1 # Normalize [-1,1]
    BA_impact = np.log(teamB_off_impact * teamA_def_impact) # team2’s FT offense vs team1’s FT defense (impact)
    BA_net_impact = 2 * ((BA_impact - min_impact_matchup) / (max_impact_matchup - min_impact_matchup)) - 1 # Normalize [-1,1]
    AB_eff = np.log(teamA_off_eff * teamB_def_eff) # team1’s FT offense vs team2’s FT defense (efficiency)
    AB_net_eff = 2 * ((AB_eff - min_eff_matchup) / (max_eff_matchup - min_eff_matchup)) - 1 # Normalize [-1,1]
    BA_eff = np.log(teamB_off_eff * teamA_def_eff) # team2’s FT offense vs team1’s FT defense (efficiency)
    BA_net_eff = 2 * ((BA_eff - min_eff_matchup) / (max_eff_matchup - min_eff_matchup)) - 1 # Normalize [-1,1]

    # DISPLAY ALL STATS AND MATCHUPS
    print("\n\n\nFREE THROW COMPARISON")
    print()
    print("NET FT IMPACT SCORE (Shows estimated FT margin for team given adjusted tempo)") # Net margin based on tempo-adjusted FT impact
    print(f"{team1 + ':':<12} {teamA_net_impact:.2f}    {team2 + ':':<12} {teamB_net_impact:.2f}") # Print both teams' net impact
    print()
    print("NET FT EFFICIENCY SCORE (Shows estimated FT margin for team given a fixed tempo)") # Net margin based on tempo-fixed FT efficiency
    print(f"{team1 + ':':<12} {teamA_net_eff:.2f}    {team2 + ':':<12} {teamB_net_eff:.2f}") # Print both teams' net efficiency
    print()
    print("HEAD TO HEAD MATCHUP")
    print("OFFENSE VS DEFENSE IMPACT SCORE (Closer to -1: Disadvantage, Closer to 0: Even, Closer to 1: Advantage)") # Explain impact score
    print(f"{team1 + ':':<12} {AB_net_impact:.2f}    {team2 + ':':<12} {BA_net_impact:.2f}") # Show impact-based matchup scores
    print()
    print("OFFENSE VS DEFENSE EFFICIENCY SCORE (Closer to -1: Disadvantage, Closer to 0: Even, Closer to 1: Advantage)") # Explain efficiency score
    print(f"{team1 + ':':<12} {AB_net_eff:.2f}    {team2 + ':':<12} {BA_net_eff:.2f}") # Show efficiency-based matchup scores
